...
Usage:
    python scripts/test_llm_classification.py [-n NUM_CASES] [--dry-run] [--use-case-id ID] [--model MODEL] [--batch-size SIZE] [--checkpoint-file FILE] [--all]
"""
...
async def process_batch(use_cases: List[Dict], analyzer: LLMAnalyzer, categories: List[Dict], 
                       db_service: Neo4jService, dry_run: bool, checkpoint_mgr: Optional[CheckpointManager] = None) -> None:
    """Process a batch of use cases with retry logic."""
...
async def run_classification(num_cases: Optional[int] = 5, dry_run: bool = False, 
    ...
    Args:
        num_cases: Number of use cases to process (None if processing all)
        dry_run: If True, don't save results to database
        use_case_id: Optional specific use case ID to process
        model: Name of the Ollama model to use
        batch_size: Number of cases to process in each batch
        checkpoint_file: Optional file to store progress
        process_all: If True, process all use cases regardless of num_cases
    ...
    try:
        ...
        logger.info(f"Starting classification test (dry_run: {dry_run}, model: {model})")
        ...
        for i in range(0, len(use_cases), batch_size):
            ...
            await process_batch(batch, analyzer, categories, db_service, dry_run, checkpoint_mgr)
